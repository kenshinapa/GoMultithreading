...
func mineWindDistribution(windsDirections []string) {
	for _, wind := range windsDirections {
...
func main() {
	...
	for _, file := range files {
		...
		metarsReport := parseToArray(text)

		// 2. Extract wind direction, EGLL 312350Z 07004KT CAVOK 12/09 Q1016 NOSIG= -> 070
		windsDirections := extractWindDirection(metarsReport)
